...
| 16 | caching-and-cdn-strategies | RESEARCHING | FOUR clusters drafted/factchecked (A placement+patterns, B eviction+sizing, C consistency+invalidation, D CDN+edge) and RECONCILED into `_research.md`; all sizing/stampede math VERIFIED by recomputation (Zipf hit-ratio H(k,a)/H(N,a), origin load (1-h), stampede R*T_r); 03/06/08/10/13/14/15 canon reused; RFC 9111/5861, Nishtala NSDI 2013, Breslau INFOCOM 1999, XFetch VLDB 2015, vendor CDN/anycast attributions `[UNVERIFIED]` network-blocked (8th session, carried forward) | brain |
