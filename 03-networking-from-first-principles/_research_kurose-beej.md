...
- **Why layering & the end-to-end argument.** Saltzer/Reed/Clark (1984): a function (e.g., reliable delivery, error recovery, encryption) often *cannot be completely or correctly* implemented by the network's lower levels; the endpoints must do it anyway, so doing it in the network is redundant cost. Hence put application-specific guarantees at the edges and keep the core simple/dumb. This is the principled justification for why IP is best-effort and TCP's reliability lives in the end hosts. Canonical: MIT PDF above. (PDF text auto-extraction failed — thesis stated from the verified abstract/citation, not a page-level quote; mark page quotes [UNVERIFIED].)
...
- **epoll/kqueue depth.** Beej teaches `select`/`poll` thoroughly; `epoll`(Linux)/`kqueue`(BSD/macOS) are the scalable production successors but are platform-specific and lightly covered. Gap: if the course targets high-concurrency servers, a non-Beej primary (man pages / Linux docs) is needed for `epoll`. [UNVERIFIED — confirm Beej's exact epoll coverage.]
