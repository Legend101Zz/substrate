...
- **QUIC CPU cost is genuinely contested.** Some measurement/vendor reports claim QUIC can use **~2×–4× the CPU of TLS/TCP** for large transfers, with substantial cost in userspace per-packet `sendmsg/recvmsg`; UDP GSO/GRO can narrow the gap. Factcheck verdict: these figures are **CITE-NEEDED** until pinned to a specific measurement paper/data source. Net: the "QUIC is slower" claim is **workload- and tuning-dependent**, not absolute. Do not teach exact percentages without a primary citation. a single canonical source]
- **HTTP/3 adoption maturity is a moving target.** Avoid embedding a stale percentage. Safer course phrasing: HTTP/3/QUIC is supported by all major browsers and major CDNs, but server-side library maturity, UDP handling by middleboxes, and operational tuning remain real caveats. For current adoption, cite HTTP Archive/W3Techs with a retrieval date.
...
**Gaps / citation-needed flags:** HTTP/3+QUIC absent from the named cluster (supplement required); QUIC CPU-cost figures (2×–4×) need a specific measurement paper/data source before use; Stevens 2nd-ed chapter numbers not re-verified; HTTP/3 adoption % and HTTP/2 server-push status are date-sensitive — cite current HTTP Archive/W3Techs/browser docs with retrieval date before teaching.
