...
`posix_spawn()` exists but "is behind the scenes implemented in terms of... fork and exec." (Evans) The two-step model is deliberate: between fork and exec the child is a full process under the shell's control, so the shell can run arbitrary setup code (dup2, close, setpgid, signal reset, chdir) *in the child's context* before handing it to the new program. A single atomic spawn would need every possible setup to be expressed as flags/file-actions up front. **Forcing constraint:** redirection, pipe plumbing, and job-control group placement are all "things done by the child to itself between fork and exec" — fork/exec separation is what makes them expressible at all.
...
- **Julia Evans, "What happens when you start a process on Linux?":** https://jvns.ca/blog/2016/10/04/exec-will-eat-your-brain/ — fork-then-exec mental model ("a child that is a clone of myself... gets its brain eaten and turns into ls"), exec replaces image, posix_spawn = fork+exec, zombies, signal/fd inheritance gotcha.
...
- **CodeCrafters exact stage count/order:** the course-definition.yml lists the canonical core 8 + many extension stages (quoting, redirection, completion, jobs, pipelines, history, declare); the "8 stages" figure in marketing refers only to the base track. Stage *names* above are paraphrased from the yml via web fetch — [UNVERIFIED] exact slugs; confirm against the raw yml before quoting verbatim in course prose.
- **`posix_spawn` internals:** Evans says it's "implemented in terms of fork and exec"; on Linux/glibc this is true historically, but some libc/platforms implement posix_spawn via a dedicated `clone`/`vfork`+exec fast path. [UNVERIFIED] for current glibc — do not assert "always fork+exec" without checking glibc source. The teaching point (you *can* think of it as fork+exec) stands regardless.
...
- CodeCrafters stage **slugs/exact wording**: [UNVERIFIED] — paraphrased via WebFetch summary of course-definition.yml, not line-quoted from raw source.
- glibc `posix_spawn` current implementation path: [UNVERIFIED].
- Julia Evans direct quotes: phrasings ("brain eaten / turns into ls", "dead but waiting around") are from WebFetch summaries of jvns.ca; verify against the live post before quoting verbatim in prose. The originally-cited jvns "how to run a program" 2016/02/20 URL 404s — the correct post is the 2016/10/04 "exec-will-eat-your-brain" one used here.
